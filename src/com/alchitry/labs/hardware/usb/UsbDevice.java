...
public class UsbDevice {
        ...
        try {
            ...
            for (Device dev : list) {
                ...
                for (UsbDescriptor udes : descriptions) {
                    if (udes.getVid() == desc.idVendor() && udes.getPid() == desc.idProduct()) {
                        vidPidMatch = true;
                        break;
                    }
                }
                if (vidPidMatch) {
                    ...
                    for (UsbDescriptor udes : descriptions) {
                        if (udes.getVid() == desc.idVendor() && udes.getPid() == desc.idProduct() && (udes.getProduct() == null || udes.getProduct().equals(sDesc))) {
                            match = udes;
                            break;
                        }
                    }
    ...
    public static void entryListFree(List<DeviceEntry> devList) {
        for (DeviceEntry dev : devList)
            LibUsb.unrefDevice(dev.getDevice());
    }
